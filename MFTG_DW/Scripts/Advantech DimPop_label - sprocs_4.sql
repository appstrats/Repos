...
--exec sp_pop_assembly_advantech_label 4
use [MFTG_DW]
go
create PROC sp_pop_assembly_advantech_label (@pLoadID int) as
begin
set nocount on
...
begin
with T_Assem as
(select distinct data_value udv from MFGTESTC_ADVANTECH.dbo.process_step_data sd
...
--exec sp_pop_serialnumber_advantech_label 4
create PROC sp_pop_serialnumber_advantech_label (@pLoadID int) as
begin
set nocount on
...
with T_SN as
(select distinct serial_number from MFGTESTC_ADVANTECH.dbo.process_step_result sr where 
...
T_Label_Auth as
(select serial_number, max(data_value) Authcode from MFGTESTC_ADVANTECH.dbo.process_step_data sd
...
--Manufacturing_ID
--exec sp_pop_manufacturing_id_advantech_label 4
create PROC sp_pop_manufacturing_id_advantech_label (@pLoadID int) as
begin
set nocount on
...
begin
with T_SN as
(select distinct serial_number from MFGTESTC_ADVANTECH.dbo.process_step_result where 
len(serial_number) = 12 and
 (serial_number like '0006B1%' or serial_number like '0017C5%' or  serial_number like 'FFFFFF%'
or serial_number like 'C0EAE4%' or serial_number like '18B169%' or serial_number like '004010%') ),
M_ID as
(select distinct  s.sn1
from MFGTESTC_ADVANTECH.[dbo].sn_relation s
...
-- exec sp_pop_firmversion_advantech_label 4
create PROC sp_pop_firmversion_advantech_label (@pLoadID int) as
begin
set nocount on
...
with T_FirmW as
(select distinct Data_Value FW from MFGTESTC_ADVANTECH.dbo.process_step_data sd
...
go
-- exec sp_pop_romversion_advantech_label 4
create PROC sp_pop_romversion_advantech_label (@pLoadID int) as
begin
set nocount on
...
begin
with T_ROMv as
(select distinct Data_Value RV from MFGTESTC_ADVANTECH.dbo.process_step_data sd
...
go
--Safemode version
-- exec sp_pop_safemodeversion_advantech_label 4
create PROC sp_pop_safemodeversion_advantech_label (@pLoadID int) as
begin
set nocount on 
...
begin
with T_SMV as
(select distinct Data_Value SMV from MFGTESTC_ADVANTECH.dbo.process_step_data sd
...
-- exec sp_pop_IRV_advantech_label 4
create PROC sp_pop_IRV_advantech_label (@pLoadID int) as
begin
set nocount on
...
with T_IRV as
(select distinct Data_Value IRV from MFGTESTC_ADVANTECH.dbo.process_step_data sd
...
go
--Regmode
-- exec sp_pop_regulatorymodel_advantech_label 6
create PROC sp_pop_regulatorymodel_advantech_label (@pLoadID int) as
begin
set nocount on
...
begin
with T_RM as
(select distinct Data_Value RM from MFGTESTC_ADVANTECH.dbo.process_step_data sd
...
go
-- exec sp_pop_station_advantech_label 4 
create PROC sp_pop_station_advantech_label (@pLoadID int) as
begin
set nocount on
...
begin
with T_S as
(select distinct(Station_id) STA from MFGTESTC_ADVANTECH.dbo.process_step_result sr where (sr.dateStamp between @startdate and @enddate )
)
...
--Location
--exec sp_pop_Location_advantech_label 4
create PROC sp_pop_Location_advantech_label (@pLoadID int) as
begin
set nocount on
declare @startdate  datetime
declare @enddate  datetime
...
from MFGTESTC_ADVANTECH.[dbo].[mfg_location] s
...
--SKU
--exec sp_pop_SKU_advantech_label 4
create PROC sp_pop_SKU_advantech_label (@pLoadID int) as
begin
set nocount on
...
begin
with T_SKU as
(select distinct '01-SSC-'+data_value SKU from MFGTESTC_ADVANTECH.dbo.process_step_data sd 
...
--Part number 
--exec sp_pop_Partnum_advantech_label 4
Create PROC sp_pop_Partnum_advantech_label (@pLoadID int) as
begin
set nocount on
declare @startdate  datetime
declare @enddate  datetime
...
from MFGTESTC_ADVANTECH.dbo.part_number s
...
--Step result Code
--exec sp_pop_Stepresult_code_advantech_label 4
create PROC sp_pop_Stepresult_code_advantech_label (@pLoadID int) as
begin
set nocount on
declare @startdate  datetime
declare @enddate  datetime
...
from MFGTESTC_ADVANTECH.[dbo].[step_result_code] s
...
-- exec sp_pop_Stationtype_advantech_label 4
create PROC sp_pop_Stationtype_advantech_label (@pLoadID int) as
begin
set nocount on
declare @startdate  datetime
declare @enddate  datetime
...
begin
with T_STC as
(select distinct (cast(station_type_code as varchar)) STC  from MFGTESTC_ADVANTECH.dbo.process_step_result sr where (sr.datestamp between @startdate and @enddate)) 
...
--exec sp_pop_RFID_advantech_label 2
create PROC sp_pop_RFID_advantech_label (@pLoadID int) as
begin
set nocount on
declare @startdate  datetime
declare @enddate  datetime
...
begin
with T_RFID as
(select distinct  data_value RFID from MFGTESTC_ADVANTECH.dbo.process_step_data sd
