...
insert into etl_configuration.dbo.Step_log(stepdesc, load_id, starttime) values ('step 1 completion', @pLoadID, getdate());
...
insert into etl_configuration.dbo.Step_log(stepdesc, load_id, starttime) values ('step 2 completion', @pLoadID, getdate());

select a.sn,a.sn_1, min(a.lvl) l_lvl into #t_l from #t a 
group by a.sn, a.sn_1

insert into etl_configuration.dbo.Step_log(stepdesc, load_id, starttime) values ('step 3 completion', @pLoadID, getdate());
...
insert into etl_configuration.dbo.Step_log(stepdesc, load_id, starttime) values ('step 4 completion', @pLoadID, getdate());
...
insert into etl_configuration.dbo.Step_log(stepdesc, load_id, starttime) values ('step 5 completion', @pLoadID, getdate());
...
insert into etl_configuration.dbo.Step_log(stepdesc, load_id, starttime) values ('step 6 completion', @pLoadID, getdate());
...
T_SKU as
(select  step_index,  data_value SKU from MFGTESTC_FREMONT.dbo.process_step_data sd
where data_attribute ='label assembly part number' and  data_value like '01-SSC%'
and step_index in
(select step_index from MFGTESTC_FREMONT.dbo.process_step_result where station_type_code in(1,4))and (sd.datestamp between @startdate and @enddate)),
T_RM as
(select  step_index,  data_value RM from MFGTESTC_FREMONT.dbo.process_step_data sd
where data_attribute like 'label field RM'and (sd.datestamp between @startdate and @enddate)),
T_RFID as
(select  step_index,  data_value RFID from MFGTESTC_FREMONT.dbo.process_step_data sd
where data_attribute like 'label field RFID'and (sd.datestamp between @startdate and @enddate)),
T_FirmW as
(select  step_index,  data_value FW from MFGTESTC_FREMONT.dbo.process_step_data sd 
where data_attribute like 'FirmwareVersion'and (sd.datestamp between @startdate and @enddate)),
T_ROMv as
(select  step_index, data_value RV from MFGTESTC_FREMONT.dbo.process_step_data sd
where data_attribute like 'ROMVersion'and (sd.datestamp between @startdate and @enddate)),
T_SMV as
(select  step_index,  data_value SMV from MFGTESTC_FREMONT.dbo.process_step_data sd
where data_attribute like 'SafeModeVersion'and (sd.datestamp between @startdate and @enddate)),
T_RMA as
(select  step_index,  'yes' RMA from MFGTESTC_FREMONT.dbo.process_step_data sd 
...
select  sr.serial_number, 
 ...
 datepart(HH,sr.datestamp) T_hour,
  1 datasourcekey, -- datasourcekey
 sr.step_index,
 dbo.fn_getdatavalue(sr.step_index, sr.serial_number,1) psdata, 
 sr.step_result_code,
 ml.location_name ,
 pn.part_number,
 ...
 into #T_fact 
  ...
  left outer join MFGTESTC_FREMONT.dbo.part_number pn on sr.pn_code = pn.pn_code
  left outer join MFGTESTC_FREMONT.dbo.sn_relation r on sr.serial_number = r.sn2 and sr.station_id = r.station_id 
