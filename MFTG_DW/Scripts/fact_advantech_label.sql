...
select distinct r.sn2,r.sn1 into #snr from MFGTESTC_ADVANTECH.dbo.sn_relation r;

with all_mid (sn, sn_1, lvl) as
(select distinct serial_number , r.sn1 , 0 lvl
from MFGTESTC_ADVANTECH.dbo.process_step_result sr
inner join #snr r
...
T_SKU as
(select  step_index,  data_value SKU from MFGTESTC_ADVANTECH.dbo.process_step_data sd
where data_attribute ='label assembly part number' and  data_value like '01-SSC%'
and step_index in
(select step_index from MFGTESTC_ADVANTECH.dbo.process_step_result where station_type_code in(1,4))and (sd.datestamp between @startdate and @enddate)),
T_RM as
(select  step_index, data_value RM from MFGTESTC_ADVANTECH.dbo.process_step_data sd
where data_attribute like 'label field RM'and (sd.datestamp between @startdate and @enddate)),
T_RFID as
(select  step_index, data_value RFID from MFGTESTC_ADVANTECH.dbo.process_step_data sd
where data_attribute like '%RFID%'and (sd.datestamp between @startdate and @enddate)),
T_FirmW as
(select Step_Index,Data_Value FW from MFGTESTC_ADVANTECH.dbo.process_step_data sd
...
T_ROMv as
(select Step_Index,Data_Value RV from MFGTESTC_ADVANTECH.dbo.process_step_data sd
where rtrim(Data_Attribute) like 'ROMVersion' and (sd.datestamp between @startdate and @enddate )),
T_SMV as
(select Step_Index,Data_Value SMV  from MFGTESTC_ADVANTECH.dbo.process_step_data sd
where rtrim(Data_Attribute )like 'SafeModeVersion' and (sd.datestamp between @startdate and @enddate ))  ,
T_RMA as
(select  step_index,  'yes' RMA from MFGTESTC_ADVANTECH.dbo.process_step_data sd 
