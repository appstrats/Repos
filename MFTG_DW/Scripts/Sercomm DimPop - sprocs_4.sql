...
if (@startdate is not null and @enddate is not null)
begin
--No mapping all 9999 found, hence not populated
print 0
--with T_PNC as
...
--exec sp_pop_RFID_Sercomm 6
create PROC sp_pop_RFID_Sercomm (@pLoadID int) as
begin
set nocount on
declare @startdate  datetime
declare @enddate  datetime
...
begin
with T_RFID as
(select distinct  data_value RFID from MES2_SERCOMM.dbo.process_step_data sd
