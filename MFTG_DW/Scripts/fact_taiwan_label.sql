use MFTG_DW
go
--exec sp_populate_fact_taiwan_Label 8
...
select distinct r.sn2,r.sn1 into #snr from MFGTESTC_TAIWAN.dbo.sn_relation r where r.[date] <=@enddate;

with all_mid (sn, sn_1, lvl) as
(select distinct serial_number , r.sn1 , 0 lvl
from MFGTESTC_TAIWAN.dbo.process_step_result sr
inner join #snr r
...
T_SKU as
(select  step_index,  data_value SKU from MFGTESTC_TAIWAN.dbo.process_step_data sd
where data_attribute ='label assembly part number' and  data_value like '01-SSC%'
and step_index in
(select step_index from MFGTESTC_TAIWAN.dbo.process_step_result where station_type_code in(1,4))and (sd.datestamp between @startdate and @enddate)),
T_RM as
(select  step_index, data_value RM from MFGTESTC_TAIWAN.dbo.process_step_data sd
where data_attribute like 'label field RM'and (sd.datestamp between @startdate and @enddate)),
T_RFID as
(select  step_index, data_value RFID from MFGTESTC_TAIWAN.dbo.process_step_data sd
where data_attribute like 'label field RFID'and (sd.datestamp between @startdate and @enddate)),
T_FirmW as
(select Step_Index,Data_Value FW from MFGTESTC_TAIWAN.dbo.process_step_data sd
...
T_ROMv as
(select Step_Index,Data_Value RV from MFGTESTC_TAIWAN.dbo.process_step_data sd
where rtrim(Data_Attribute) like 'ROMVersion' and (sd.datestamp between @startdate and @enddate )),
T_SMV as
(select Step_Index,Data_Value SMV  from MFGTESTC_TAIWAN.dbo.process_step_data sd
where rtrim(Data_Attribute )like 'SafeModeVersion' and (sd.datestamp between @startdate and @enddate ))  ,
T_RMA as
(select  step_index,  'yes' RMA from MFGTESTC_TAIWAN.dbo.process_step_data sd 
...
select  sr.serial_number, 
 ...
 ml.location_name ,
 sr.pn_code,
 cast(null as varchar(250)) part_number,
...
into #T_fact  
  ...
  left outer join T_RFID rfid on sr.step_index = rfid.step_index --and sr.serial_number = rm.serial_number
  ...
  left outer join MFGTESTC_TAIWAN.dbo.sn_relation r on sr.serial_number = r.sn2 and sr.station_id = r.station_id 
  left outer join MFGTESTC_TAIWAN.dbo.mfg_location ml on sr.location_code = ml.location_code 
...
update sr set sr.part_number = pn.part_number, sr.pn_desc = pn.[Description] from #T_fact sr
  left outer join MFGTESTC_TAIWAN.dbo.part_number pn on sr.pn_code = pn.pn_code   
