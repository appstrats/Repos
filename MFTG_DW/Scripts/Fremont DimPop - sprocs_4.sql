...
Create PROC sp_pop_assembly_FREMONT (@pLoadID int) as
begin
set nocount on
...
(select distinct data_value udv from MFGTESTC_FREMONT.dbo.process_step_data sr
...
--As
Create PROC sp_pop_serialnum_FREMONT (@pLoadID int) as
begin
set nocount on
...
with T_SN as
(select distinct serial_number from MFGTESTC_FREMONT.dbo.process_step_result sr where 
...
T_Label_Auth as
(select serial_number, max(data_value) Authcode from MFGTESTC_FREMONT.dbo.process_step_data sd
...
Create PROC sp_pop_Manufacturing_ID_FREMONT (@pLoadID int) as
begin
set nocount on
...
begin
with T_SN as
(select distinct serial_number from MFGTESTC_FREMONT.dbo.process_step_result where 
len(serial_number) = 12 and
 (serial_number like '0006B1%' or serial_number like '0017C5%' or  serial_number like 'FFFFFF%'
or serial_number like 'C0EAE4%' or serial_number like '18B169%' or serial_number like '004010%') ),
M_ID as
(select distinct  s.sn1
from [MFGTESTC_FREMONT].[dbo].sn_relation s
...
Create PROC sp_pop_SKU_FREMONT (@pLoadID int) as
begin
set nocount on
...
begin
with T_SKU as
(select distinct  data_value SKU from MFGTESTC_FREMONT.dbo.process_step_data sd
...
union 
select distinct '01-SSC-'+data_value SKU from MFGTESTC_FREMONT.dbo.process_step_data sd 
...
Create PROC sp_pop_Regulatory_Model_FREMONT (@pLoadID int) as
begin
set nocount on
declare @startdate  datetime
declare @enddate  datetime
...
begin
with T_RM as
(select distinct data_value RM from MFGTESTC_FREMONT.dbo.process_step_data sd
...
Create PROC sp_pop_Partnum_FREMONT (@pLoadID int) as
begin
set nocount on
declare @startdate  datetime
declare @enddate  datetime
...
Create PROC sp_Stepresult_code_FREMONT (@pLoadID int) as
begin
set nocount on
declare @startdate  datetime
declare @enddate  datetime
...
from [MFGTESTC_FREMONT].[dbo].[step_result_code] s
...
Create PROC sp_pop_Firmware_FREMONT (@pLoadID int) as
begin
set nocount on
declare @startdate  datetime
declare @enddate  datetime
...
begin
with T_FirmW as
(select distinct data_value FW from MFGTESTC_FREMONT.dbo.process_step_data sd 
...
Create PROC sp_pop_ROMV_FREMONT (@pLoadID int) as
begin
set nocount on
declare @startdate  datetime
declare @enddate  datetime
...
begin
with T_ROMv as
(select distinct data_value RV from MFGTESTC_FREMONT.dbo.process_step_data sd
...
Create PROC sp_pop_safemodever_FREMONT (@pLoadID int) as
begin
set nocount on
declare @startdate  datetime
declare @enddate  datetime
...
begin
with T_SMV as
(select distinct data_value SMV from MFGTESTC_FREMONT.dbo.process_step_data sd
...
Create PROC sp_pop_Station_FREMONT (@pLoadID int) as
begin
set nocount on
declare @startdate  datetime
declare @enddate  datetime
...
begin
with T_S as
(select distinct(Station_id) STA from MFGTESTC_FREMONT.dbo.process_step_result sr  where (sr.datestamp between @startdate and @enddate))
...
StationTypeKey>-1),0) + ROW_NUMBER() over (ORDER BY s.STC ), s.STC
from T_STC s
...
Create PROC sp_pop_datecode_FREMONT (@pLoadID int) as
begin
set nocount on
declare @startdate  datetime
declare @enddate  datetime
...
begin
with T_DC as
(select distinct format(datestamp,'yyyyMMdd') DC  from MFGTESTC_FREMONT.dbo.process_step_result sr  where (sr.datestamp between @startdate and @enddate) )
