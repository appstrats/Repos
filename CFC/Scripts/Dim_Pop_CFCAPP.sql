...
left outer join [CFC_DW].sysdba.Account d on s.CustID  = d.ACCOUNTID
...
left outer join [CFC_DW].sysdba.Ship_Region d on s.CID  = d.Region
...
left outer join [CFC_DW].sysdba.PRODUCT d on s.ivt  = d.PRODUCT_NUMBER
...
from T_Label_User1 s
left outer join [CFC_DW].sysdba.User d on s.User1  = d.Name
